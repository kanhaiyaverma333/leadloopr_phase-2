// lib/facebookAds/getValidAccessToken.ts
...
export async function getValidAccessToken(orgId: string): Promise<string> {
  ...
  // Facebook tokens can be refreshed by exchanging for new long-lived tokens
  ...
  if (!tokenRes.ok) {
    console.error("🔴 Failed to refresh Facebook token", {
    ...
    throw new Error("Unable to refresh Facebook Ads token");
  }
