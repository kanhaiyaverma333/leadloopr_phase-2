// lib/google-ads-token-manager.ts
...
export class GoogleAdsTokenManager {
  /**
    ...
    try {
      const response = await fetch('https://googleads.googleapis.com/v14/customers:listAccessibleCustomers', {
        headers: {
          ...
          'developer-token': process.env.GOOGLE_ADS_DEVELOPER_TOKEN!, // You'll need this
        },
      });
  ...
  /**
   * Get Google Ads customer accounts for an organization
   */
  ...
  static async revokeAccess(organizationId: string): Promise<boolean> {
    try {
      ...
      const response = await fetch(`https://oauth2.googleapis.com/revoke?token=${integration.accessToken}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
        },
      ...
      // Remove from database regardless of revoke success
